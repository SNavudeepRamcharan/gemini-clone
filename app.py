...
SYSTEM_RULE = (
    "You are Next AI Pro, an exceptionally advanced and precise computing intelligence model. "
    ...
    "Never guess or give generic answers. Provide highly technical, comprehensive, and step-by-step solutions "
    "with clean structure styling."
)

# In-memory session tracking structure
chat_database = {}
...
def chat():
    ...
    if not api_key:
        return jsonify({"error": "GEMINI_API_KEY environment variable is not set on your hosting platform."}), 500
    if not user_message and not uploaded_file:
        return jsonify({"error": "No prompt or operational file received."}), 400
    ...
    if uploaded_file and uploaded_file.filename != '':
        try:
            ...
            return jsonify({"error": f"Failed to process multi-modal file asset: {str(file_err)}"}), 400
    ...
    # Force zero temperature creativity settings for maximum reasoning stability
    payload = {
        "contents": contents,
    ...
    try:
        ...
        if 'candidates' not in response_data:
            return jsonify({"error": f"API Error Logged: {response_data}"}), 400
        ...
        return jsonify({"error": f"API Server Route Down: {str(e)}"}), 500
